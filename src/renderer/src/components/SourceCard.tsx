  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 pr-5">
        ...
        {isSelected && <span className="text-[9px] px-1.5 py-0.5 rounded bg-accent-blue/20 text-accent-blue border border-accent-blue/30 shrink-0">{t('sourceSelected')}</span>}
      </div>
      ...
      <div className="flex items-center justify-between gap-1">
        <div className="flex flex-col">
          <span className="text-[9px] text-gray-500 uppercase tracking-wider">{t('sourceUsed')}</span>
        ...
        <div className="flex flex-col items-end">
          <span className="text-[9px] text-gray-500 uppercase tracking-wider">{t('sourceRemaining')}</span>
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] text-gray-600 font-mono">{t('sourceTotal')} {fmt(vol.totalBytes)}</span>
      </div>
    </div>
  )
}
    ...
    <div className="flex flex-col flex-1 min-w-0 gap-1.5">
      ...
      <div className="relative group shrink-0 w-fit">
        <label className="no-drag flex items-center gap-2.5 cursor-pointer">
          ...
          <span className="text-xs text-gray-400">{t('fx3RenameLabel')}</span>
        </label>
        <div className="pointer-events-none absolute left-0 top-full mt-1.5 z-50 w-64 rounded-lg bg-gray-900 border border-border shadow-xl px-3 py-2 text-xs text-gray-300 leading-relaxed opacity-0 group-hover:opacity-100 transition-opacity">
          <p className="font-medium text-gray-200 mb-1">{t('fx3RenameTooltipTitle')}</p>
          <p>{t('fx3RenameTooltipDesc')}</p>
        </div>
      </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {volumes.length === 0 ? (
            <div onClick={onPickSource}
              ...
              <span className="text-xs text-center px-4">{t('noDiskDetected')}</span>
              <button
